...
- If another DNS proxy is *not* running, modify your network settings to use our proxy's listening address for the operating system's resolver. This can typically be done by modifying the file `/etc/resolv.conf`. On MacOS, you will need to use the Network configuration panel, which seems to only accept addresses on port 53. If you wish to use `127.0.0.1:53`, you may need to disable `mDNSResponder`. See the more detailed instructions linked below. 
