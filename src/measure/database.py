...
class DNSDatabase:
    ...
    def insert_har(self, experiment, domain, recursive, dns_type, multi_trr_model, har, har_uuid, har_error, delays):
        insert = ''' INSERT INTO {}
                     (uuid, experiment, insertion_time, domain, recursive, dns_type, multi_trr_model, har, error, delays)
        ...
        if har:
            tup = (har_uuid, experiment, now, domain, recursive,
                   dns_type, multi_trr_model, Json(har), None, Json(delays))
        else:
            tup = (har_uuid, experiment, now, domain, recursive,
                   dns_type, multi_trr_model, None, har_error, Json(delays))
    ...
    def insert_dns(self, har_uuid, experiment, recursive,
                   dns_type, multi_trr_model, all_dns_info):
        insert = '''INSERT INTO {}
                    (har_uuid, experiment,
        ...
        for resolver_ip in all_dns_info:
            per_resolver_info = all_dns_info[resolver_ip]
            for domain in per_resolver_info:
                dns_info = per_resolver_info[domain]
                now = datetime.datetime.utcnow()
                recursive = dns_info['recursive']
    ...
    def get_unique_uuids(self, recursive, dns_type, domains):
        domains = tuple(domains)
        cmd = '''
            SELECT distinct uuid, experiment, insertion_time, domain, recursive, dns_type, multi_trr_model
            FROM {}
            WHERE recursive = %s AND
                  dns_type  = %s AND
                  domain IN %s
        ...
        if experiments:
            ...
            cmd = '''
                SELECT uuid, domain, experiment,
                       dns_type, recursive, multi_trr_model,
    ...
    def get_resources_by_model(self, domains, model, experiments=None):
        domains = tuple(domains)
        if experiments:
            experiments = tuple(experiments)
            cmd = '''
                SELECT uuid, domain, experiment,
                       ...
                       (jsonb_extract_path(jsonb_array_elements(jsonb_extract_path(har, 'entries')), 'request')->>'url') as url
                FROM {}
                WHERE domain IN %s AND multi_trr_model=%s AND experiment IN %s
                '''.format(self.har_table)
            rv = self._execute_command(cmd, (domains, experiments))
        else:
            cmd = '''
                SELECT uuid, domain, experiment,
                       ...
                       (jsonb_extract_path(jsonb_array_elements(jsonb_extract_path(har, 'entries')), 'serverIPAddress')) as serverIPAddress
                FROM {}
                WHERE domain IN %s AND multi_trr_model=%s
                '''.format(self.har_table)
            rv = self._execute_command(cmd, (domains, model))
        ...
        if experiments:
            ...
            cmd = '''
                SELECT har_uuid, domain, experiment, dns_type, recursive,
                       multi_trr_model, response_time, response_size, error
                FROM {}
                WHERE domain IN %s AND experiment IN %s
                '''.format(self.dns_table)
            rv = self._execute_command(cmd, (domains, experiments))
